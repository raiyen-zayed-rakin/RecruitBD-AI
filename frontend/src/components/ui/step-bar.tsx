      ...
      {VIEWS.map((s, i) => (
        <div key={i} className="flex items-center gap-2">
          <div className="flex items-center gap-2">
            ...
            <span
              className={`text-xs transition-colors first-letter:uppercase ${i === current ? "font-semibold" : "text-muted-foreground"} `}
            >
              {s}
            </span>
          </div>
          {i < VIEWS.length - 1 && (
            <div
              className={`mr-2 h-px w-8 transition-colors ${i < current ? "bg-primary" : "bg-border"} `}
            ></div>
          )}
        </div>
      ))}
